...
describe("[ReservoirV6_0_1] Filling listings and bids via the SDK", () => {
  ...
  it("Fill multiple Seaport private listings - direct fill", async () => {
    const buyer = dan;
    ...
    const echange = new Sdk.SeaportV15.Exchange(chainId);
    const tx = await echange.fillOrdersTx(
      buyer.address,
      listings.map((c) => c.order as Sdk.SeaportV15.Order),
      listings.map(() => ({})),
      {
