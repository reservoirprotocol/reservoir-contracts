...
describe("PaymentProcessorV2.1 - SingleToken Erc721", () => {
  ...
  it("Build and fill sell order", async () => {
    ...
    const builder = new PaymentProcessorV21.Builders.SingleToken(chainId);
    const orderParameters = {
      protocol: PaymentProcessorV21.Types.OrderProtocols.ERC721_FILL_OR_KILL,
  ...
  it("Build and direct fill multiple sell orders", async () => {
    ...
    // Approve the exchange
    await nft.approve(seller, PaymentProcessorV21.Addresses.Exchange[chainId]);
    await nft2.approve(seller, PaymentProcessorV21.Addresses.Exchange[chainId]);
  ...
  it("Bulk-signing", async () => {
    ...
    await exchange.bulkSign(buyer, [buyOrder, buyOrder2]);
