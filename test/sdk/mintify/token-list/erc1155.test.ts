...
import * as Mintify from "@reservoir0x/sdk/src/mintify";
...
describe("Mintify - TokenList ERC1155", () => {
  ...
  it("Build and fill buy order with partial filling", async () => {
    ...
    await weth.approve(buyer, Mintify.Addresses.Exchange[chainId]);

    // Approve the exchange for the sellers
    await weth.approve(seller1, Mintify.Addresses.Exchange[chainId]);
    await weth.approve(seller2, Mintify.Addresses.Exchange[chainId]);
    ...
    await nft.approve(seller1, Mintify.Addresses.Exchange[chainId]);
    await nft.approve(seller2, Mintify.Addresses.Exchange[chainId]);

    const exchange = new Mintify.Exchange(chainId);
